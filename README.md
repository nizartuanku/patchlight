...
Patchlight can explain a finding in plain language with a small language model that runs on
your own hardware. It is off by default. Turn it on by starting a
[hexward-ai](https://github.com/nizartuanku/hexward-ai) sidecar and pointing Patchlight at it:

```sh
patchlight -ai-assist-url http://127.0.0.1:8435
```
...
- **The engine still decides.** The model receives one finding after Patchlight has produced it.
...
Environment equivalents: `PATCHLIGHT_AI_ASSIST_URL`, `PATCHLIGHT_AI_ASSIST_KEY_FILE`,
`PATCHLIGHT_AI_ASSIST_LANG`, `PATCHLIGHT_AI_ASSIST_NO_THINKING=1`.
